 ...
 * @copyright   Copyright 2016 Kingkod
 *
...
/*                var id=datos[0];
                ...
                db.transaction(function(transaction) {
                 var executeQuery = "UPDATE usuarios SET nombre=?,login=? WHERE id=?";
                 transaction.executeSql(executeQuery, [nombre,login,id],
                 //On Success
...
//Disabling submit button
      ...
      <strong>Fail!</strong> Error al grabar.\
      </div>';
      $('#altEditor-modal .modal-body').append(message);                  
                })
...
/*  .done (function(data) { 
    ...
    if(data === "false"){
      ...
      $('#altEditor-modal .modal-body').append(message); 
    }else{$('#altEditor-modal .modal-body .alert').remove();
