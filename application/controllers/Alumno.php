...
//	$sedenombre = $this->session->userdata('nombre');
	...
	if ($this->session->userdata('where_grilla') != null) {
		...
		$arreglo = $this->Lectivo_model->Get_Alumnos($where,$cuantos);  
	}	

	$data['alumnos'] = $arreglo;
	$data['clave'] = 0;
...
public function mostrar_grilla($anio, $clave, $dni, $apellido, $nombre) {  
	...
	$alumnos = $this->Lectivo_model->Get_Alumnos($where,$cuantos);  
	$data['error'] = '';
	$data['esgrilla'] = 'S';			
...
/*			$cuantos = 30;
//			$alumnos = $this->Lectivo_model->Lectivo_por_dni($dni,$cuantos);  
			$where = $this->obtener_where_grilla($anio, $clave, $dni, $apellido, $nombre);
			$alumnos = $this->Lectivo_model->Get_Alumnos($where,$cuantos);  
			$data['error'] = '';
			$data['esgrilla'] = 'S';			
...
//				$data2['nombre'] = $this->session->userdata('nombre');
				...
				if (($clave == '') && ($anio == '') && ($dni == '') && ($apellido == '') && ($nombre == '')) {
					...
					if (($clave == '') or ($anio == '')) {
						...
						$alumno = $this->Lectivo_model->Get_Alumnos($where,2); 
						if (count($alumno) != 1) { 
							$this->mostrar_grilla($anio, $clave, $dni, $apellido, $nombre);
							return;
						}
...
//							$data['error'] = 'No registra asignacion';
					...
					$data['datos'] = $this->Lectivo_model->Datos_Padron($anio,$clave);
					if ($anio == ANIO_TOTAL) {
						$data['total'] = $this->Lectivo_model->Datos_Total($clave);
					}
					else {
						$data['total'] = array();
					}
