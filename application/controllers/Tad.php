...
class Tad extends CI_Controller {
    ...
    //		$this->load->library('Sendmail'); 
        $this->envio_mails_habilitado = false;
        $config = array(
...
public function Traducir_Nombre_Mail($t_mail,$t_tramite){
  if ($t_tramite == 'I') {
    ...
    if (($t_tramite == 'J') or ($t_tramite == 'DDJJ')) {
      switch ($t_mail) {
        case 'X':
            return 'Ya esta en guarani plan 2000 - No en padron (no requiere ddjj)';
            break;
        case 'Q':
          return 'Ya esta en guarani plan 2000 y en padron (no requiere ddjj)';
          break;
        case 'R':
            return "Ingreso mas de un tramite";
            break;
        case 'O':
            return "No en padron ni en guarani (requiere tad1)";
            break;
        default:
          return $t_mail;
        }
...
public function obtener_where_tad($dni, $apellido, $nombre, $email, $estado) {  
  ...
  if ($nombre != '') {
      ...
      $where = $where  . ' NOMBRE_SOLICITANTE like "'. $nombre . '%"';
  }	
  if ($estado != '') {
      if ($where != '') { $where = $where . ' and ';}
      if ($estado == 'G') {
        $where = $where  . " estado in ('I','G') ";
      } 
      else {
...
public function Consulta_post() {  
  ...
  /*            $cuantos = count($datos);
    ...
    if ($expediente == '') {
      ...
      $t_tramite = $datos[0]['t_tramite'];
    }
    $datos =  $this->Tad_model->Get_un_Tad($expediente,$t_tramite); 
    if (count($datos) > 0) {
...
public function InsertarEnTemporal($datos,$id_archivo,$cruce,$t_tramite,$cruce_g){
  ...
  /*    $datos['mail_enviado_cbc'] = 'P';   
    ...
    if ((!($this->envio_mails_habilitado)) && ($datos['mail_enviado_cbc'] == 'P')) {
      $datos['mail_enviado_cbc'] = 'X';
    }
