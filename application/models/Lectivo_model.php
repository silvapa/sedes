...
/*  public function Datos_Padron($anio,$clave){
  ...
  public function Datos_Padron_Actual($clave){
    $query = $this->db->query("select p.*, c.nombre as d_carrera, s.descripcion as d_nacionalidad,
    case when sede1 = 0 or sede1 is null then '' else (select descripcion from sedes where sede = p.sede1) end as d_sede1, 
    case when sede2 = 0 or sede2 is null then '' else (select descripcion from sedes where sede = p.sede2) end as d_sede2  
    from padron_actual p left join rv_insc i on p.anio = i.anio and p.clave = i.clave left join carreras c on p.carrera = c.carrera left join paises s on p.nacionalidad = s.codigo ". 
    " where p.clave = ".$clave);
    $result = $query->result_array();
    return $result;
  }
...
*/
  ...
  public function Get_Alumnos_padron_actual($where_total, $cuantos){
    $result = array();
    $select = "select ".ANIO_TOTAL." as anio,clave,apellido,nombre,dni,carrera,cuil,grupo, sede1,turno1,sede2,turno2,
    case when opta = -1 then ' ' else opta end as opta from padron_actual ";
    if (($where_total != '') && ($where_total != 'false')) {
        $select = $select ."  where ".$where_total;
    }    
    if ($cuantos > 0) {
      $select = $select ." limit ".$cuantos;
