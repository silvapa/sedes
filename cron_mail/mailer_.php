<?php 
   ...
   $t_mail = 'MD01';
   include('mail_'.$t_mail.'.php'); // cuerpo del mail a enviar
	//---------------------------------------------------------------------------------
   $myfile = fopen("logfile".$t_mail.".txt", "a");
   //----------------------------------------------------------------------------------
   $debug = true;
   $httpdirs = "https://www.cbc.uba.ar";
   $tema = "CBC Campus Virtual";  // titulo del mail
   ...
   fwrite($myfile, $txt);
   $sql = "select * from mailer where t_mail = '".$t_mail."' and estado = 'P' limit 600"; // query para levantar desde tad1
